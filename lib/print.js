...
export default async function (m, conn = { user: {} }) {
 ...
 console.log(`${chalk.hex('#FF0000').bold('╭────═[ Emilia Re:zero - MD ]═─────⋆')}
${chalk.hex('#FF0000').bold('│╭───────────────···')}
││👨‍💻 ${chalk.cyan('%s')} ⏱️ ${chalk.black(chalk.bgBlue('%s'))} 📂 ${chalk.black(chalk.bgGreen('%s'))} ${chalk.magenta('%s [%s %sB]')} 
││🥷 ${chalk.redBright('%s')} 🏦 ${chalk.yellow('%s%s')} ${chalk.blueBright('en')} 
││💻 ${chalk.cyan('%s')} ${chalk.black(chalk.bgBlue('%s'))}
${chalk.hex('#FF0000').bold('│╰────────────────···')}
${chalk.hex('#FF0000').bold('╰───────────═┅═──────────')}`.trim(), 
