...
let categoria = "video"
const sb = './media/database/db.json'
const sb_ = JSON.parse(fs.readFileSync(sb))
const randomvideo = Math.floor(Math.randomvideo() * db_.links[categoria].length)
const randomvid = db_.links[categoria][randomvideo]
const respon = await fetch(randomvid)
const vid = await respon.buffer()
global.videos = vid
