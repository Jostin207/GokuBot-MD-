...
if (!fs.existsSync("./ZeroJadiBot/" + authFolderB)) {
fs.mkdirSync("./ZeroJadiBot/" + authFolderB, { recursive: true });
}
if (args[0]) {
fs.writeFileSync("./ZeroJadiBot/" + authFolderB + "/creds.json", JSON.stringify(JSON.parse(Buffer.from(args[0], "base64").toString("utf-8")), null, '\t'));
}
...
if (code !== DisconnectReason.connectionClosed) { parent.sendMessage(m.chat, { text: "☁️ Conexión perdida con el servidor." }, { quoted: m });
}}
...
handler.command = ['code', 'sercode', 'botcode'];
